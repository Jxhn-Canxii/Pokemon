<template>
  <div v-if="hasPermission('Navigation_Buildings')">
    <div class="flex flex-col space-y-6 p-1 min-h-screen">
      <!-- Breadcrumbs and Header -->
      <div class="flex items-center justify-between">
        <BreadCrumbs page="Building" />
      </div>
      ...
      <UCard>
        <div class="flex flex-col md:flex-row md:justify-between items-center gap-4">
          ...
          />
          <AddBuilding @transaction_id="handleTransactions" />
        </div>

        <!-- Responsive Table Wrapper -->
        <div class="overflow-x-auto">
          <UCard>
            <UTable class="text-nowrap" loading loading-color="primary" loading-animation="carousel" :rows="loading ? [] : data.buildings" :columns="columns">
              ...
              <template #body>
                ...
                <tr v-else-if="data.buildings.length === 0">
              ...
              <template #buildingName-data="{ row }">
                {{ row.building }} ({{ row.buildingAbbre }})
              </template>
              <template #actions-data="{ row }">
                <div class="flex space-x-2">
                  <DeleteBuilding :key="row.id" :data="row" @transaction_id="handleTransactions" />
                </div>
              </template>
            </UTable>
          </UCard>
...
// Modules
import AddBuilding from "./module/AddBuilding.vue";
import DeleteBuilding from "./module/DeleteBuilding.vue";
...
const columns = [
  ...
  { key: "ourCompanyName", label: "Company" },
  { key: "ourCompanyTin", label: "TIN" },
...
const getBuilding = async () => {
  try {
    ...
    const response = await axios.post(
      `${VUE_APP_API_URL}buildings/list`,
      search.value,
      BearToken()
    );
...
const handleTransactions = () => getBuilding();

// Load permissions on mount
onMounted(getBuilding);
</script>
