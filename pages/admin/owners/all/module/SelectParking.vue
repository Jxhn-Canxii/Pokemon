<template>
  <div>
    <UButton icon="i-heroicons-truck" color="blue" @click.prevent="openModal()">
      Select Parking
    </UButton>

    <Modalx v-model:show="isAddModalOpen" title="Select Parking" @close="isAddModalOpen = false" :maxWidth="'4xl'">
      <UCard>
        <UTable class="text-wrap text-xs" :rows="loading ? [] : data.parkings" :columns="columns">
...
const openModal = () => {
  ...
  getParkings(); // Clear the form when opening the modal
};
...
const getParkings = async () => {
  try {
    ...
    const response = await axios.post(
      `${VUE_APP_API_URL}parkings/get-vacant`,
      search.value,
      BearToken()
    );
