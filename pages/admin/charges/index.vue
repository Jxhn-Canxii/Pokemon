<template>
  <div v-if="hasPermission('Navigation_Charges')">
    <div class="flex flex-col space-y-6 p-1 min-h-screen">
      <!-- Breadcrumbs and Header -->
      <div class="flex items-center justify-between">
        <BreadCrumbs page="Charges" />
      </div>
      ...
      <UCard>
        <div class="flex flex-col md:flex-row md:justify-between items-center gap-4">
          ...
          />
          <AddCharge @transaction_id="handleTransactions" />
        </div>

        <!-- Responsive Table Wrapper -->
        <div class="overflow-x-auto">
          <UCard>
            <UTable class="text-nowrap" loading loading-color="primary" loading-animation="carousel" :rows="loading ? [] : data.charges" :columns="columns">
              ...
              <template #body>
                ...
                <tr v-else-if="data.charges.length === 0">
              ...
              <template #actions-data="{ row }">
                <div class="flex space-x-2">
                  <DeleteCharge :key="row.id" :data="row" @transaction_id="handleTransactions" />
                </div>
              </template>
            </UTable>
          </UCard>
...
// Modules
import AddCharge from "./module/AddCharge.vue";
import DeleteCharge from "./module/DeleteCharge.vue";
...
const handleTransactions = () => getCharges();

// Load permissions on mount
onMounted(getCharges);
</script>
