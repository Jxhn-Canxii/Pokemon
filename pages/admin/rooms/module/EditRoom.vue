<template>
  <div>
    <UButton icon="i-heroicons-pencil-square" color="yellow" @click="openModal()">
      Edit
    </UButton>

    <Modalx v-model:show="isEditModalOpen" title="Edit Room" @close="isEditModalOpen = false" :maxWidth="'4xl'">
      <UCard>
...
const fillForm = () => {
  ...
  form.value = {
    ...
    room_price: parseFloat(data.roomPrice ?? 0),
    service_price: parseFloat(data.servicePrice ?? 0),
    balcony_price: parseFloat(data.balconyPrice ?? 0),
    room_area: parseFloat(data.roomArea ?? 0),
    service_area: parseFloat(data.serviceArea ?? 0),
    balcony_area: parseFloat(data.balconyArea ?? 0),
  };
  errors.value = {};
};
