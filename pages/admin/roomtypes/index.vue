<template>
  <div v-if="hasPermission('Navigation_RoomTypes')">
    <div class="flex flex-col space-y-6 p-1 min-h-screen">
      <!-- Breadcrumbs and Header -->
      <div class="flex items-center justify-between">
        <BreadCrumbs page="Room Types" />
      </div>
      ...
      <UCard>
        <div class="flex flex-col md:flex-row md:justify-between items-center gap-4">
          ...
          />
          <AddRoomType @transaction_id="handleTransactions" />
        </div>

        <!-- Responsive Table Wrapper -->
        <div class="overflow-x-auto">
          <UCard>
            <UTable class="text-nowrap" loading loading-color="primary" loading-animation="carousel" :rows="loading ? [] : data.roomtypes" :columns="columns">
              ...
              <template #body>
                ...
                <tr v-else-if="data.roomtypes.length === 0">
              ...
              <template #actions-data="{ row }">
                <div class="flex space-x-2">
                  <DeleteRoomType :key="row.id" :data="row" @transaction_id="handleTransactions" />
                </div>
              </template>
            </UTable>
          </UCard>
...
// Modules
import AddRoomType from "./module/AddRoomType.vue";
import DeleteRoomType from "./module/DeleteRoomType.vue";
...
const getRoomTypes = async () => {
  try {
    ...
    const response = await axios.post(
      `${VUE_APP_API_URL}roomtypes/list`,
      search.value,
      BearToken()
    );
...
const handleTransactions = () => getRoomTypes();
