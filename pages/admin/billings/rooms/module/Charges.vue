<template>
  <div>
    <UButton icon="i-heroicons-banknotes" color="lime" @click="openModal()">
      Charges
    </UButton>

    <Modalx v-model:show="isEditModalOpen" title="Billing Charges" @close="isEditModalOpen = false" maxWidth="fullscreen">
      <UCard>
        <UForm>
          <div class="space-y-6 p-4 overflow-auto">
            ...
            <UCard>
                ...
                <div class="flex justify-between items-center">
                  ...
                  <AddCondoDues
                    :data="form"
                    :key="form.room_id"
                    :condoDuesDropdown="dues_dropdown"
                    @transaction_id="handleTransactions()"
                  />
                </div>
                  ...
                  <template #actions-data="{ row, index }">
                    <div class="flex space-x-2">
                      <UButton
                        ...
                        size="sm"
                        label="Delete"
                       @click.prevent="deleteBillingCharges(row.id)"
                      />
                      <UButton
                        color="red"
                        v-else
                        ...
                        size="sm"
                        label="Remove"
                        @click.prevent="removeChargeRow(row)"
                      />
                    </div>
                  </template>
                </UTable>
            ...
            <UCard>
                <div class="flex justify-between items-center">
                   ...
                   <AddParkingCharges :key="form.room_id" :parkingsDropdown="parkings_dropdown" :data="form" @transaction_id="handleTransactions()" />
                </div>
                <!-- rooms table list -->
                <UTable class="text-nowrap" loading-animation="carousel" :rows="loading ? [] : parkings_data" :columns="parkingColumns">
                  ...
                  <template #actions-data="{ row, index }">
                    <div class="flex space-x-2">
                      <UButton
                        :color="row.isCapture == 1 ? 'yellow' : 'red'"
                        icon="i-heroicons-trash"
                        variant="ghost"
                        size="sm"
                        :disabled="row.isCapture == 1"
                        label="Delete"
                       @click.prevent="deleteBillingCharges(row.id)"
                      />
                       <!-- <UButton
            ...
            <UCard>
                <div class="flex justify-between items-center">
                     ...
                     <AddUtilityCharges :key="form.room_id" :data="form" @transaction_id="handleTransactions()" />
                </div>
                <!-- rooms table list -->
                <UTable class="text-nowrap" loading-animation="carousel" :rows="loading ? [] : readings_data" :columns="readingsColumns">
                  <template #actions-data="{ row, index }">
                    <div class="flex space-x-2">
                      <UButton
                        :color="row.isCapture == 1 ? 'yellow' : 'red'"
                        ...
                        :disabled="row.isCapture == 1"
                        label="Delete"
                        @click.prevent="deleteBillingCharges(row.charge_bill_id)"
                      />
                    </div>
                  </template>
                </UTable>
            ...
            <UCard>
                <div class="flex justify-between items-center">
                    ...
                    <AddMisc :data="form" :key="form.room_id"  @transaction_id="handleTransactions()" />
                </div>
                <!-- rooms table list -->
                <UTable class="text-nowrap" loading-animation="carousel" :rows="loading ? [] : misc_data" :columns="miscColumns">
                   <template #remarksText-data="{ row, index }">
                    <p class="text-wrap">{{ row.remarks }}</p>
                  </template>
                  <template #actions-data="{ row, index }">
                    <div class="flex space-x-2">
                      <UButton
...
import AddCondoDues from "./AddCondoDues.vue";
import AddUtilityCharges from "./AddUtilityCharges.vue";
import AddParkingCharges from "./AddParkingCharges.vue";
import AddMisc from "./AddMisc.vue";
...
const getBillingCharges = async () => {
  try {
    ...
    const response = await axios.post(
      `${VUE_APP_API_URL}billings/get-charge-bills-per-room`,
      formData, 
      BearToken()
    );
    ...
    form.value.previous_balance = roomInfo.previousBalance ?? 0;
...
const deleteBillingCharges = async (charge_id) => {
  ...
  if (prompt.isConfirmed) {
    try {
      ...
      const response = await axios.post(`${VUE_APP_API_URL}billings/delete-charge-bill`, formData, BearToken());
