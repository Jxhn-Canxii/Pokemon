<template>
  <div>
    <UButton icon="i-heroicons-arrows-right-left" color="red" @click="openModal()">
      Transfer
    </UButton>

    <Modalx v-model:show="isAddModalOpen" title="Change Room Owner" @close="isAddModalOpen = false" :maxWidth="'4xl'">
      <UCard>
        <UForm @submit.prevent="transferOwner">
...
const fillCustomerForm = () => {
  ...
  console.log(data);
  form.value.guest_id = data.ownerId;
  form.value.account_num = data.ownerAccountNumber;
  form.value.id_number = data.ownerId;
  form.value.fullname = data.ownerName; 
  form.value.email = data.ownerEmail; 
  ...
  form.value.contact = data.contact_info; 
  form.value.address = data.owneraAddress ?? ""; 
  form.value.remarks = data.ownerRemarks ?? "";
  errors.value = {};
};
