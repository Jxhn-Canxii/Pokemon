<template>
  <div>
    <UButton icon="i-heroicons-truck" color="blue" @click="openModal()">
      Parkings
    </UButton>

    <Modalx v-model:show="isEditModalOpen" title="Parking's Owned" @close="isEditModalOpen = false" :maxWidth="'4xl'">
      <UCard>
        <UForm>
          <div class="space-y-6 p-4 overflow-auto">
            ...
            <!-- Rates Section -->
            <div>
                <h3 class="text-lg font-semibold mb-4">Parkings List</h3>
                <!-- rooms table list -->
...
const columns = [
  { key: "parkingName", label: "Parking", sortable: true  },
...
const getRoomsPerOwner = async () => {
  try {
    ...
    const response = await axios.post(
      `${VUE_APP_API_URL}owners/get-parkings-per-owner`,
      search.value,
      BearToken()
    );
